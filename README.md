...
若感觉主动回复过于频繁：可在设置页开启「明确接话请求也交给判断模型」（否则「在吗 / 说句话」这类请求根本不会走判断模型），确认「冷却秒数」与 Dashboard 里的「每日上限」不是 0（0 表示不限制），必要时调大「最小静默秒数」。`/selfreply status` 会显示最近一次裁决原因，可据此区分是判断模型放行、还是明确请求直通。
