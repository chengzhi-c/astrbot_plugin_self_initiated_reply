...
<p align="center">
  ...
  <img alt="版本" src="https://img.shields.io/badge/版本-1.4.0-4a5580">
...
| 明确接话请求也交给判断模型 | 回复偏频繁时开启 | 「在吗 / 有人吗 / 说句话 / 发表情包」这类明确请求默认直接接话、不经过判断模型；开启后同样先由判断模型裁决 |
...
若感觉主动回复过于频繁：可在设置页开启「明确接话请求也交给判断模型」（否则「在吗 / 说句话」这类请求根本不会走判断模型），确认「冷却秒数」与 Dashboard 里的「每日上限」不是 0（0 表示不限制），必要时调大「最小静默秒数」。`/selfreply status` 的「最近裁决」一行会显示本会话最近一次裁决的时间、结论（接话/不接话）与原因，可据此区分是判断模型放行、还是明确请求直通。

> 图片下载固定 `trust_env=False`（不读取系统/环境变量代理），以避免宿主代理配置影响识图。这是有意设计：若你的网络必须经代理才能访问图片 URL，识图会静默失败（降级为纯文本回复），此时需自行确保图片地址可直连。
