...
- 运行开关、私聊主动回复开关、新消息放弃旧回复开关与白名单概况；
- 上下文 / 延迟 / 静默 / 冷却参数；
...
改完点「保存」，页面会回读服务端配置。保存请求带配置 revision；如果请求超时或配置已被其他页面修改，页面会显示状态未知/冲突并要求先刷新，不会用旧 revision 自动重试覆盖新配置。被后端截断、去重或归一化的字段会在保存提示中列出。重载插件或重启 AstrBot 后仍想看到最新页面资源，请硬刷新浏览器。
...
| 冷却秒数 | 同一会话两次主动回复至少间隔 `300 – 900` 秒（默认 `900`） |
| 新消息到达时放弃未发出的旧回复 | 默认关闭；群聊很吵、不想旧回复再发出时再开 |
