...
判定依据：**任何改动不得破坏下列行为**。红线测试围绕这些条目设计；条目变更
必须显式修订本文档并附理由。

哪些条目曾被逐个变异实测证明「承重」（改掉即有测试变红）：0.9.3 前由
`scripts/mutation_check.py` 的 **38 条**变异定义承载（与 CHANGELOG 的
「mutation 38/38 全击杀」同源），该脚本已删除（549 行自研变异工具对单插件仓
不成比例，且它自己的 docstring 承认只做锚点串替换、不生成变异体、不算变异分数）。
原始清单不需要人工转录副本，直接从 git 取：

```
git show 27d8864:scripts/mutation_check.py   # 看第 29 行起的 MUTANTS 表
```

表项形如 `(名称, 目标文件, 原始串, 变异串, -k 表达式)`，覆盖 14 个文件。
**注意它已部分失效**：表内含 `state_saver.py`（0.9.3 已删除，其职责并入
`storage.py` 的落盘路径与 §8 的原子写契约），且其余锚点串也会随重构漂移。
它是历史证据，不是可直接重跑的门禁。
需要真变异测试时用 `mutmut`，不要复活这个脚本。

0.9.3 起改用**逐项手工变异验证**替代：新增测试必须先把被测逻辑改坏、
确认该测试变红、再恢复，验证记录写在当次批次的复审结论里。
理由是自研工具维护的是锚点串（会漂移），手工验证锚定的是语义（不会）。
...
## 11. 共享容器身份（0.9.3 阶段 0 / 阶段 5）

main 在装配段把若干可变容器（dict/set）的**引用**交给协作对象
（scheduler / session_coordinator / whitelist / gate）。这些对象在构造时捕获的是
容器对象本身，不是 main 的属性名。由此产生两条承重契约：

- **B1 — 容器只能原地改，绝不属性重绑定。** 任何恢复/重置路径（当前唯一入口是
  `webapi._restore_plugin_state`）必须用 `clear()` + 原地写入
