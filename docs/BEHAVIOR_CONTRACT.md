...
- 会话失效（白名单移除/手动检查/插件停止，以及开启
  `abandon_stale_on_new_message` 时的新消息）走同一失效入口，级联清理
  事件缓存、图片索引与任务注册。
- `abandon_stale_on_new_message` 默认关。关闭时入口不因新消息推进代次，
  正在生成、尚未发出的主动回复继续走完；新消息仍记录上下文、刷新下一轮
  静默并重排延迟检查。开启后恢复旧语义：同一会话任何被入口接住的消息
  （含表情包、单独符号、空内容）都会推进代次，未发出的旧回复作废。白名单
  ABA、指令检查和停止路径不受此开关影响。
...
3. 静默时间（`min_silence_sec`）：按本次检查开始时的活动时间计算。生成途中
   到达的新消息刷新下一轮静默，不得拦下本轮已通过的发送。
4. 冷却（`cooldown_sec`）
5. 观察窗口已推进（该事件后已回复过）

免打扰、日配额、冷却、观察窗口发送前重查最新状态。decorating hook await 返回后，事件发送前还必须重新检查代次和插件生命周期；停止发生在 hook 内时不得继续 `event.send`。
...
- `abandon_stale_on_new_message` 默认关。关闭时入口不因新消息推进代次；
  正在生成、尚未发出的主动回复继续走完。新消息仍记录上下文、刷新下一轮
  静默，并重排尚未开始检查的延迟任务。
- 开启后：同一会话任何被入口接住的消息（含表情包、单独符号、空内容）推进代次，
  未发出的旧回复作废。
- 白名单移除、`/selfreply check`、插件停止仍走 `invalidate`，不受此开关影响。
- 不按消息类型拆第二套代次策略。
...
- POST `/config` 若带 `base_revision`，必须在 `_config_lock` 内先与当前摘要比较；不一致返回 `error_code=STALE_WRITE` 和当前 `config_revision`，不得应用部分更新。成功响应返回完整 `config`、新的 `config_revision` 和 `adjusted_fields`。不带 `base_revision` 的旧调用继续串行写入，不承诺防止迟到旧写覆盖。
...
- 同一份裁决原因在聊天指令 `/selfreply status` 里也呈现（时间、接话/不接话、原因；
  原因折单行并截断 60 字）。两处同源于 `session_pipeline.record_decision` 的单点写入，
  不得出现第二份判据或第二次格式化口径；聊天窗口那份同样是只读的（不创建会话状态）。

## 14. 引用、明确请求与"已被回应"判定

- **引用（`quote_mode`）默认关。** 开启后引用的是**本次检查依据的那条消息**
  （`_last_events[umo]` 的 message_id，与判断提示词的 `{latest_message}` 同源）：
  - `model`：判断模型在裁决 JSON 里给出可选 `quote` 决定；模型未表态（明确请求
    直通、手动检查、判断模型关闭）时按 `quote_probability` 兜底。`quote` 解析不出
...
- **`skip_after_direct_call` 默认开。** 被 @Bot/唤醒的消息由 AstrBot 正常回复
  （不经过本插件），插件把它记为「这条消息之后 Bot 已经回应过」：同一批消息不再
  触发主动回复，`last_active_at` 被下一条新消息推进后自动恢复。关闭后回到旧行为
  （只更新活跃时间，不推进观察窗口）。
- **`reply_request_requires_model` 默认关。** 明确请求（在吗 / 有人吗 / 说句话 /
  发表情包）默认直通判断、不询问判断模型（那类句子的意图本身已明确）；开启后这些
  请求同样交给判断模型，模型的否决必须生效。直通与强制路径的 `quote` 为 None。
- 判断提示词里的聊天记录变量超预算时**保尾**（保留最新若干条），与生成路径
  `cap_context_text` 同口径；截头会丢掉最新几条，与默认模板「优先参考最近至少
  8 条」的要求相反。
